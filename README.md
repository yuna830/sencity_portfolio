...
## 아키텍처
![시스템 아키텍처](./assets/architecture1.png)
...
#### AI 분석 모듈(코드는 Django 폴더에)
  + EfficientNet-B1(이미지 분류)
      + 자동 분석으로 오인 신고 감소, 실시간 위험 감지, 정책 기반 데이터 축적
      + 사용자 신고 이미지 기반 동물 종 분류
  + YOLOv5s(객체 탐지)
      + CCTV 영상에서 야생동물 객체 탐지
      + 탐지 결과를 서버로 전달하여 실시간 정보 반영
